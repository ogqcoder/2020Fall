...
async function follow(id, followerid) {
    const sql = `INSERT INTO ${PREFIX}Followers (User_id, Follower_id) VALUES ? ;`;
    const params = [[id, followerid]];
    return await mysql.query(sql, [params]);
}
async function unfollow(id, followerid) {
    const sql =
        `Delete FROM ${PREFIX}Followers    
        WHERE  User_id = ? AND Follower_id = ?
    `;
    console.log(id, followerid);
    return await mysql.query(sql, [id, followerid]);
}

// async function update(id, FirstName, LastName, Password, User_Type) {
//     const sql = `UPDATE ${PREFIX}Users SET ? WHERE id = ?;`;
//     const params = { FirstName, LastName, Password, User_Type };
